...
static void __init setup_command_line(char *command_line)
{
	...
	if (extra_init_args) {
		extra_init_args =
			strim(extra_init_args); /* remove trailing space */
...
static int run_init_process(const char *init_filename)
{
	...
	file = filp_open(init_filename, O_RDONLY, 0);
	if (IS_ERR(file)) {
		pr_err("Could not open file %s\n", init_filename);
		return PTR_ERR(file);
	}

	bytes_read = kernel_read(file, buf, 20, &pos);
	if (bytes_read < 0) {
	...
	buf[bytes_read] = '\0';
	pr_info("First 20 characters of %s: %s\n", init_filename, buf);
