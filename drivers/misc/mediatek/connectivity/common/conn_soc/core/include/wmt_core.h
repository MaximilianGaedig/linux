...
/*
 * Manual antenna-switch control TLV, sent to firmware during coex init so
 * it actually uses the ANT_SEL0/1 GPIO switch this board's RF frontend
 * depends on - ported back from Amazon's kernel (originally gated behind
 * CONFIG_MTK_WIFI_ANTENNA_SWITCH, which doesn't exist in this tree; this
 * board always needs it, so it's unconditional here instead of adding a
 * single-value Kconfig option).
 */
