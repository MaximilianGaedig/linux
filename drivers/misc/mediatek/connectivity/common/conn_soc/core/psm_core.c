...
#else
/* struct timeval/do_gettimeofday() removed (y2038 cleanup); use timespec64. */
...
INT32 stp_psm_disable_by_tx_rx_density(MTKSTP_PSM_T *stp_psm, INT32 dir, INT32 length)
{
	if (sample_start) {
		...
		*/
		if (((tv_now.tv_sec == tv_end.tv_sec) &&
		     (tv_now.tv_nsec / NSEC_PER_USEC > tv_end.tv_nsec / NSEC_PER_USEC)) ||
