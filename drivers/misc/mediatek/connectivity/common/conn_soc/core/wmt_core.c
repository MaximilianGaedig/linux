...
INT32 wmt_core_init_script(struct init_script *script, INT32 count)
{
	...
	for (i = 0; i < count; i++) {
		...
		pr_info("biscuit-initscript: [%d/%d] %s tx ok, waiting for rx\n", i, count, script[i].str);
		/* EVENT BUF */
		osal_memset(evtBuf, 0, sizeof(evtBuf));
		iRet = wmt_core_rx(evtBuf, script[i].evtSz, &u4Res);
		pr_info("biscuit-initscript: [%d/%d] %s rx returned iRet=%d u4Res=%u\n",
			i, count, script[i].str, iRet, u4Res);
		...
		if (0x14 != evtBuf[1]) {	/* workaround RF calibration data EVT,do not care this EVT */
			...
			/*
			 * The RF calibration EVT this workaround skips content
			 * checking for is a fixed 6-byte ack on some chip/fw
			 * combos, but on this hardware the chip answers with a
			 * much larger (~630-byte) calibration data dump instead.
			 * wmt_core_rx() above only drained script[i].evtSz (6)
			 * bytes of it, so hundreds of leftover calibration bytes
			 * are still sitting in the WMT_TASK_INDX rx ring queue,
			 * about to be misread as the response to the *next*
			 * init_script command and desync everything after it.
			 * Flush them before continuing.
			 */
