...
#endif
static UINT8 WMT_COEX_SETTING_CUSTOM_FEATURE_EVT[] = { 0x02, 0x10, 0x01, 0x00, 0x00 };
...
#endif
	INIT_CMD(NULL, WMT_COEX_SETTING_CUSTOM_FEATURE_EVT, "coex_custom_feature"),
};
...
#define COEX_PTA  3
#define COEX_MISC 4
#define COEX_CUSTOM_FEATURE 5
#else
#define COEX_CUSTOM_FEATURE 1
#endif
	...
	/*
	 * Tell firmware to actually use manual antenna-switch control (the
	 * ANT_SEL0/1 GPIO switch) - without this, the switch's SoC-side pin
	 * config alone isn't enough; firmware defaults to not driving RF
	 * through it, which matches this board's observed symptom of a
	 * fully working HCI/STP protocol layer with zero real over-the-air
	 * RX/TX. See struct coex_custom_feature's declaration for context.
	 */
	...
	for (i = 0; i < COEX_CUSTOM_MAX; i++) {
		...
		case COEX_CUSTOM_MANUAL_ANT_CTRL:
			if (pWmtGenConf->coex_custom_feature_manual_ant_ctrl) {
				...
				if (idx + custom_data.len <= sizeof(buffer)) {
					...
					osal_memcpy(&buffer[idx], custom_data.data, custom_data.len);
	...
	/*
	 * wmt_core_init_script() has no NULL-cmd guard - if data_len stays
	 * 0 (e.g. coex_custom_feature_manual_ant_ctrl unset), leaving this
	 * entry's INIT_CMD(NULL, ...) cmdSz (sizeof(NULL), i.e. pointer
	 * size) unchanged would hand a NULL pointer of nonzero length to
	 * wmt_core_tx(). Zero the size too so that call is a safe no-op.
	 */
	coex_table[COEX_CUSTOM_FEATURE].cmdSz = 0;
	...
	if (custom_cmd.data_len) {
		...
		coex_table[COEX_CUSTOM_FEATURE].cmdSz = custom_cmd.cmd_len + 4;
