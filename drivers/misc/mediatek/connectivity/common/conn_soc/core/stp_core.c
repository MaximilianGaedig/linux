...
#else
	/*
	 * STP_PSM_CORE(stp_core_ctx) is NULL whenever stp_psm_init() bailed
	 * out early during mtk_wcn_stp_init() (FIFO/thread/monitor alloc
	 * failure) - mtk_wcn_stp_is_ready() only reflects STP's own open
	 * state, not whether PSM itself finished initializing, so the two
	 * can disagree. Calling stp_psm_enable(NULL, ...) walked straight
	 * into a NULL deref inside osal_wake_lock_count() the first time
	 * this ran. Power-save monitoring is optional (see the "not ready"
	 * fallback right below, which already treats it as skippable), so
	 * degrade the same way here instead of crashing.
	 */
	if (STP_PSM_CORE(stp_core_ctx) == NULL) {
		STP_WARN_FUNC("PSM core not initialized, skipping\n");
		return -1;
	}
	if (mtk_wcn_stp_is_ready() && mtk_wcn_stp_is_btif_fullset_mode()) {
		return stp_psm_enable(STP_PSM_CORE(stp_core_ctx), idle_time_to_sleep);
