...
/*
 * (Historical note: this used to carry a hand-written forward declaration of
 * mtk_wcn_wmt_ic_info_get(), because the declaration in
 * common_detect/wmt_stp_exp.h -- which wmt_exp.h nest-includes above --
 * mysteriously never became visible.  Root cause: essentially that entire
 * header, lines 50 to 616 including ENUM_WMT_CHIPINFO_TYPE_T and this
 * function, sits inside "#ifdef MTK_WCN_WMT_STP_EXP_SYMBOL_ABSTRACT", and
 * nothing was defining that macro because the parent
 * drivers/misc/mediatek/connectivity/Makefile that is supposed to set it was
 * not part of the vendored source.  With that Makefile added the declaration
 * arrives normally and the workaround is gone.)
 */
...
/*
 * Real bug, not API drift: this was "const PUINT8 data".  PUINT8 is a
 * typedef for UINT8 *, so "const PUINT8" declares a *const pointer to
 * mutable data* (UINT8 * const), not a pointer to const data.  The STP
 * core's MTK_WCN_STP_IF_RX callback type is void (*)(const UINT8 *, INT32)
 * -- pointer to const -- so this never matched, it just went unnoticed
 * because the two conflicting typedefs of MTK_WCN_STP_IF_RX (conn_soc's
 * include/stp_exp.h vs common_detect's wmt_stp_exp.h, which repeats the
 * same const-in-the-wrong-place mistake) are rarely in scope together.
 */
static void
mtk_bt_hci_receive(const UINT8 *data, INT32 size)
{
	int err;
