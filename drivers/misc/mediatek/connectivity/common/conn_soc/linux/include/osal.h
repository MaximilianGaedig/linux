...
typedef struct _OSAL_WAKE_LOCK_ {
	/*
	 * Modern (6.x-era) mainline only has the pointer-returning
	 * wakeup_source_register()/_unregister() API -- the older
	 * in-place wakeup_source_init()/_trash() helpers this driver
	 * originally targeted were removed. There is no in-tree
	 * <linux/wakelock.h> (that was Android-common-kernel only), so
	 * we always go through struct wakeup_source now.
	 */
