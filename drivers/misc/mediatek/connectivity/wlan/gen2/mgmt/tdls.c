...
static VOID TdlsCmdTestRxIndicatePkts(GLUE_INFO_T *prGlueInfo, struct sk_buff *prSkb)
{
	...
	/* netif_rx_ni() was removed in v5.18; netif_rx() itself now handles
	 * being called from process context (it disables bh internally), so
	 * the context split is no longer needed. */
	netif_rx(prSkb);
}
...
TdlsTestTdlsFrameSend(ADAPTER_T *prAdapter, VOID *pvSetBuffer, UINT_32 u4SetBufferLen, UINT_32 *pu4SetInfoLen)
{
	...
	TdlsexCfg80211TdlsMgmt(prWdev->wiphy, NULL,
			prCmd->arRspAddr, -1, prCmd->ucFmeType, 1,
			0, 0, /* open/none */
			FALSE, NULL, 0);
...
/*----------------------------------------------------------------------------*/
int
/* ->tdls_mgmt gained an "int link_id" after the peer address (v6.0 MLO
 * rework); -1 means "not an MLO link".  Ignored -- single-link hardware. */
