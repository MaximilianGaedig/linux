...
/*
 * Note on the extra arguments the cfg80211 ops below grew:
 *   - "int link_id" / "unsigned int link_id": which link of an 802.11be
 *     multi-link device the call applies to (v6.0 MLO rework).  -1 or 0 for
 *     non-MLO.  This is single-link 802.11n hardware, so it is ignored.
 *   - "int radio_idx": which radio of a multi-radio wiphy (recent
 *     multi-radio-per-wiphy support).  One radio here, so also ignored.
 */
...
int mtk_p2p_cfg80211_get_txpower(struct wiphy *wiphy, struct wireless_dev *wdev, int radio_idx,
...
int mtk_p2p_cfg80211_set_channel(struct wiphy *wiphy, struct net_device *dev,
...
#else
/*
 * CONFIG_NL80211_TESTMODE was removed from mainline (nl80211 testmode
 * was deprecated in favor of vendor commands). Every actual use of the
 * testmode ioctl functions elsewhere in this driver is already
 * consistently guarded by #ifdef CONFIG_NL80211_TESTMODE, so simply
 * never defining it compiles P2P/Wi-Fi-Direct testmode support out
 * cleanly -- this #error was just a deliberate build-time reminder,
 * not a real hard dependency. Left disabled rather than reimplemented
 * as a cfg80211 vendor command (a real feature gap, not just a build fix).
 */
#endif
