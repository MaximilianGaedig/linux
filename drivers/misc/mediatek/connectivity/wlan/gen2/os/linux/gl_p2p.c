...
/* see wlanSelectQueue() in gl_init.c for why the signature changed */
...
BOOLEAN glRegisterP2P(P_GLUE_INFO_T prGlueInfo, const char *prDevName, BOOLEAN fgIsApMode)
{
	...
	*(P_GLUE_INFO_T *) wiphy_priv(gprP2pWdev->wiphy) = prGlueInfo;
	...
	if (fgIsApMode) {
		...
		/*
		 * dev_change_name() is a net/core-internal symbol, not
		 * exported for driver use in this kernel - dynamically
		 * renaming the P2P netdev between AP_MODE_INF_NAME and
		 * P2P_MODE_INF_NAME is convenience-only (not needed for
		 * basic STA-mode association on the primary wiphy), so
		 * it's dropped here rather than exporting an internal
		 * core networking symbol just for this.
		 */
	} else {
...
#if CFG_SUPPORT_PERSIST_NETDEV
		prGlueInfo->prP2PInfo->prDevHandler = gprP2pWdev->netdev;
