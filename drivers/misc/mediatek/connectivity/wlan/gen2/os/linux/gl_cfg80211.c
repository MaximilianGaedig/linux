 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_cfg80211_del_key(struct wiphy *wiphy, struct net_device *ndev, int link_id,
...
/*----------------------------------------------------------------------------*/
int
mtk_cfg80211_set_default_key(struct wiphy *wiphy, struct net_device *ndev, int link_id,
 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_cfg80211_set_default_mgmt_key(struct wiphy *wiphy, struct net_device *netdev, int link_id,
				      u8 key_index)
{
	return 0;
}
				...
				const u8 *mac, struct station_parameters *params)
{
	...
	if ((params->link_sta_params.supported_rates != NULL) && (params->link_sta_params.supported_rates_len != 0)) {
		UINT32 u4Idx;

		DBGLOG(TDLS, INFO, "<tdls_cmd> %s: sup rate = 0x", __func__);

		rIeSup.ucId = ELEM_ID_SUP_RATES;
		rIeSup.ucLength = params->link_sta_params.supported_rates_len;
		for (u4Idx = 0; u4Idx < rIeSup.ucLength; u4Idx++) {
			rIeSup.aucSupportedRates[u4Idx] = params->link_sta_params.supported_rates[u4Idx];
...
/*----------------------------------------------------------------------------*/
/*!
 * @brief cfg80211 handler: management frame registrations changed.
 *
 * This replaces the old .mgmt_frame_register callback, which was removed in
 * v5.8.  The two are shaped quite differently:
 *
 *   - .mgmt_frame_register was an *edge* notification: cfg80211 called it once
 *     per (frame_type, register/unregister) transition, passing the full
 *     16-bit frame control value (e.g. 0x0040 for probe request) and a bool.
 *     A driver was expected to maintain its own registration refcount/state.
 *
 *   - .update_mgmt_frame_registrations is a *level* notification: cfg80211
 *     recomputes the complete set of registered management frame subtypes and
 *     hands us the resulting bitmaps, indexed by BIT(frame_type >> 4) -- i.e.
 *     by 802.11 frame subtype, so probe request is BIT(4) and action is
 *     BIT(13).  It also splits per-interface from device-global registrations
 *     (and unicast from multicast RX), which the old callback could not
 *     express.  The callback may sleep and may be called with no change at
 *     all, so we have to diff against our own state ourselves.
 *
 * Mapping onto this driver: it has exactly two RX management-frame filter
 * bits, probe request and action frame, both per-interface, so we rebuild
 * those two bits from upd->interface_stypes and only poke the main thread
 * when the result actually differs from what the firmware has.
 */
/*----------------------------------------------------------------------------*/
						  ...
						  IN struct mgmt_frame_regs *upd)
{
	...
	u4NewFilter = prGlueInfo->u4OsMgmtFrameFilter &
	    ~(PARAM_PACKET_FILTER_PROBE_REQ | PARAM_PACKET_FILTER_ACTION_FRAME);

	if (upd->interface_stypes & BIT(MAC_FRAME_PROBE_REQ >> 4))
		u4NewFilter |= PARAM_PACKET_FILTER_PROBE_REQ;

	if (upd->interface_stypes & BIT(MAC_FRAME_ACTION >> 4))
		u4NewFilter |= PARAM_PACKET_FILTER_ACTION_FRAME;

	DBGLOG(REQ, LOUD, "%s: stypes if=0x%08x global=0x%08x, filter 0x%08x -> 0x%08x\n",
			  __func__, upd->interface_stypes, upd->global_stypes,
			  prGlueInfo->u4OsMgmtFrameFilter, u4NewFilter);

	if (u4NewFilter == prGlueInfo->u4OsMgmtFrameFilter)
		return;
...
}				/* mtk_cfg80211_update_mgmt_frame_registrations */
