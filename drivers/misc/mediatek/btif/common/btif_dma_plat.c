...
/*vFIFO data has been read by DMA controller, just disable tx dma's irq*/
			...
			BTIF_ERR_FUNC
			    ("**********************ERROR, ERROR, ERROR**************************\n");
			BTIF_ERR_FUNC
			/* the original printed end_timer.tv_usec twice and
			 * never printed end_timer.tv_sec at all */
			    ("BTIF Tx IRQ happened %d times (continiously), between %lld.%06ld and %lld.%06ld\n",
			     MAX_CONTINIOUS_TIMES,
			     (long long)start_timer.tv_sec, start_timer.tv_nsec / 1000,
			     (long long)end_timer.tv_sec, end_timer.tv_nsec / 1000);
		}
