...
static void filename_to_taskname(char *tcomm, const char *fn,
                                 unsigned int len) {
  int i, ch;
...
void would_dump(struct linux_binprm *bprm, struct file *file) {
  if (inode_permission2(file->f_path.mnt, file->f_path.dentry->d_inode,
                        MAY_READ) < 0)
...
#ifdef CONFIG_COMPAT
int compat_do_execve(char *filename, compat_uptr_t __user *__argv,
                     compat_uptr_t __user *__envp, struct pt_regs *regs) {
  struct user_arg_ptr argv = {
      .is_compat = true,
      .ptr.compat = __argv,
  };
...
void do_coredump(long signr, int exit_code, struct pt_regs *regs) {
  ...
  if (ispipe) {
    ...
    if (cprm.limit == 1) {
       ...
       */
      printk(KERN_WARNING "Process %d(%s) has RLIMIT_CORE set to 1\n",
    ...
    retval =
        call_usermodehelper_fns(helper_argv[0], helper_argv, NULL,
                                UMH_WAIT_EXEC, umh_pipe_setup, NULL, &cprm);
