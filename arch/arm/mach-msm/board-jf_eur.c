...
enum { SNS_PWR_OFF, SNS_PWR_ON, SNS_PWR_KEEP };
#endif
...
static int atoi(char *str) {
  ...
  while (str[count] != (int)NULL && str[count] >= '0' && str[count] <= 'z') {
...
static int __init sec_tsp_mode(char *mode) {
  ...
  printk(KERN_ERR "%s: LCD_ID = 0x%s, val: 0X%x, ret1: 0x%x", __func__, mode,
         ret, ret1);
  ...
  pr_info("%s : %s", __func__, sec_tsp_synaptics_mode ? "synaptics" : "atmel");
...
static void max77693_haptic_power_onoff(int onoff) {
  ...
  if (onoff) {
    ...
    printk(KERN_ERR "haptic power_on is finished.\n");
  } else {
    ...
    printk(KERN_ERR "haptic power_off is finished.\n");
  }
}
#endif
...
static void irda_wake_en(bool onoff) {
  gpio_direction_output(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_IRDA_WAKE), onoff);
...
static void irda_device_init(void) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_IRDA_IRQ), &irda_irq);
...
static int apq8064_paddr_to_memtype(unsigned int paddr) { return MEMTYPE_EBI1; }
...
static void __init apq8064_reserve_fixed_area(unsigned long fixed_area_size) {
  ...
  pr_info("mem_map: fixed_area reserved at 0x%lx with size 0x%lx\n",
          reserve_info->fixed_area_start, reserve_info->fixed_area_size);
  BUG_ON(ret);
#endif
}
...
static void __init reserve_ion_memory(void) {
  ...
  for (i = 0; i < apq8064_ion_pdata.nr; ++i) {
    ...
    if (heap->extra_data) {
      ...
      case ION_HEAP_TYPE_CP:
        if (((struct ion_cp_heap_pdata *)heap->extra_data)->is_cma) {
          heap->size = ALIGN(heap->size, cma_alignment);
          use_cma = 1;
        }
        fixed_position =
            ((struct ion_cp_heap_pdata *)heap->extra_data)->fixed_position;
        break;
      ...
      case ION_HEAP_TYPE_CARVEOUT:
        fixed_position =
            ((struct ion_co_heap_pdata *)heap->extra_data)->fixed_position;
        break;
      default:
        break;
      }
      ...
      if (fixed_position == FIXED_LOW) {
         ...
         */
        dma_declare_contiguous(heap->priv, heap->size, 0, 0xb0000000);
      }
    }
  }
  ...
  if (low_use_cma) {
    ...
    ret = memblock_remove(fixed_low_start, fixed_low_size + HOLE_SIZE);
    pr_info("mem_map: fixed_low_area reserved at 0x%lx with size \
				0x%x\n",
            fixed_low_start, fixed_low_size + HOLE_SIZE);
    BUG_ON(ret);
  }
  ...
  if (middle_use_cma) {
    ...
    pr_info("mem_map: fixed_middle_area reserved at 0x%lx with \
				size 0x%x\n",
            fixed_middle_start, fixed_middle_size);
    BUG_ON(ret);
  }
  ...
  if (high_use_cma) {
    ...
    pr_info("mem_map: fixed_high_area reserved at 0x%lx with size \
				0x%x\n",
            fixed_high_start, fixed_high_size);
    BUG_ON(ret);
  }
  ...
  for (i = 0; i < apq8064_ion_pdata.nr; ++i) {
    ...
    if (heap->extra_data) {
      ...
      switch ((int)heap->type) {
      case ION_HEAP_TYPE_CP:
        pdata = (struct ion_cp_heap_pdata *)heap->extra_data;
      ...
      case FIXED_MIDDLE:
        ...
        if (middle_use_cma) {
          ret = dma_declare_contiguous(heap->priv, heap->size,
                                       fixed_middle_start, 0xa0000000);
          WARN_ON(ret);
        }
...
static void __init reserve_cache_dump_memory(void) {
  ...
  total = apq8064_cache_dump_pdata.l1_size + apq8064_cache_dump_pdata.l2_size;
...
static void touchkey_register_callback(void *cb) { tk_charger_callbacks = cb; }
...
GPIO_CFG_ENABLE);
                ...
                gpio_tlmm_config(GPIO_CFG(GPIO_TOUCHKEY_SCL_2, 0,
...
static void msm8960_mhl_gpio_init(void) {
  ...
  gpio_tlmm_config(GPIO_CFG(GPIO_MHL_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,
                            GPIO_CFG_2MA),
                   1);
}
  ...
  if (onoff) {
    ...
    if (system_rev >= 1) {
      ...
      if (mhl_l32 == NULL) {
        ...
        if (IS_ERR(mhl_l32))
          return;
        rc = regulator_set_voltage(mhl_l32, 3300000, 3300000);
      }
...
static int usb_diag_update_pid_and_serial_num(uint32_t pid, const char *snum) {
  ...
  printk(KERN_ERR "%s: dload:%p pid:%x serial_num:%s\n", __func__, dload, pid,
         snum);
  /* update pid */
...
static int phy_init_seq[] = {0x44,
                             0x80, /* set VBUS valid threshold
                                       and disconnect valid threshold */
                             0x6F,
                             0x81, /* update DC voltage level */
                             0x3C,
                             0x82, /* set preemphasis and rise/fall time */
                             0x17,
                             0x83, /* set source impedance adjusment */
                             -1};
...
static int initialize_ssp_gpio(void) {
  ...
  if (system_rev >= BOARD_REV03) {
    gpio_tlmm_config(GPIO_CFG(GPIO_MCU_CHG, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_2MA),
                     1);
...
static int check_ap_rev(void) { return system_rev; }
...
static void ssp_get_positions(int *acc, int *mag) {
  ...
  if (system_rev > BOARD_REV06 && samsung_hardware != SPH_L720 &&
...
static void bcm2079x_sw_i2c_config(void) {
  pr_info("%s is called\n", __func__);
  gpio_tlmm_config(
      GPIO_CFG(10, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
  gpio_tlmm_config(
      GPIO_CFG(11, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
}
#endif
...
static int __init bcm2079x_init(void) {
  ...
  if (system_rev > BOARD_REV11)
    gpio_tlmm_config(GPIO_CFG(GPIO_NFC_FIRMWARE_AP, 0, GPIO_CFG_OUTPUT,
                              GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
                     1);
  else
...
static void barcode_emul_poweron(int onoff) {
  ...
  if (barcode_vreg_2p85 == NULL) {
    ...
    ret = regulator_set_voltage(barcode_vreg_2p85, 2850000, 2850000);
    if (ret)
  ...
  if (system_rev >= BOARD_REV02) {
    if (barcode_vreg_l33 == NULL) {
      ...
      if (IS_ERR(barcode_vreg_l33))
        return;
      ret = regulator_set_voltage(barcode_vreg_l33, 1200000, 1200000);
      if (ret)
...
static void barcode_gpio_config(void) {
  ...
  pr_info("%s\n", __func__);
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_FPGA_RST_N), &creset_b);
  ...
  if (system_rev > BOARD_REV05) {
    pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_FPGA_CRESET_B),
                       &creset_b);
  } else {
    gpio_request_one(GPIO_FPGA_CRESET_B, GPIOF_OUT_INIT_LOW, "FPGA_CRESET_B");
  }
  ...
  if (system_rev >= BOARD_REV09) {
    gpio_tlmm_config(GPIO_CFG(GPIO_FPGA_MAIN_CLK, 2, GPIO_CFG_OUTPUT,
                              GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
                     GPIO_CFG_ENABLE);
  }
}
...
static void __init apq8064_ehci_host_init(void) {
  if (machine_is_apq8064_liquid() || machine_is_mpq8064_cdp() ||
    ...
    apq8064_device_ehci_host3.dev.platform_data = &msm_ehci_host_pdata3;
    platform_device_register(&apq8064_device_ehci_host3);

#ifdef CONFIG_USB_EHCI_MSM_HOST4
    apq8064_device_ehci_host4.dev.platform_data = &msm_ehci_host_pdata4;
...
static struct epm_chan_properties ads_adc_channel_data[] = {
    {10, 100},   {1000, 1},  {10, 100},  {1000, 1}, {10, 100},  {1000, 1},
    {10, 100},   {1000, 1},  {10, 100},  {20, 100}, {500, 100}, {5, 100},
    {1000, 1},   {200, 100}, {50, 100},  {10, 100}, {510, 100}, {50, 100},
    {20, 100},   {100, 100}, {510, 100}, {20, 100}, {50, 100},  {200, 100},
    {10, 100},   {20, 100},  {1000, 1},  {10, 100}, {200, 100}, {510, 100},
    {1000, 100}, {200, 100},
};
...
static int enable_100KHz_ls(int enable) {
  int ret = 0;
  if (enable) {
    ret = gpio_request(SX150X_GPIO(1, 10), "cs8427_100KHZ_ENABLE");
    if (ret) {
      pr_err("%s: Failed to request gpio %d\n", __func__, SX150X_GPIO(1, 10));
      return ret;
    }
...
static struct tsens_platform_data apq_tsens_pdata = {
    ...
    .hw_type = APQ_8064,
    .tsens_num_sensor = 11,
    .slope = {1176, 1176, 1154, 1176, 1111, 1132, 1132, 1199, 1132, 1199, 1132},
};
...
static uint8_t spm_power_collapse_without_rpm[] __initdata = {
    0x00, 0x24, 0x54, 0x10, 0x09, 0x03, 0x01,
    0x10, 0x54, 0x30, 0x0C, 0x24, 0x30, 0x0f,
};

static uint8_t spm_retention_cmd_sequence[] __initdata = {
    0x00, 0x05, 0x03, 0x0D, 0x0B, 0x00, 0x0f,
};

static uint8_t spm_retention_with_krait_v3_cmd_sequence[] __initdata = {
    0x42, 0x1B, 0x00, 0x05, 0x03, 0x0D, 0x0B, 0x00, 0x42, 0x1B, 0x0f,
};

static uint8_t spm_power_collapse_with_rpm[] __initdata = {
    0x00, 0x24, 0x54, 0x10, 0x09, 0x07, 0x01, 0x0B,
    0x10, 0x54, 0x30, 0x0C, 0x24, 0x30, 0x0f,
};

/* 8064AB has a different command to assert apc_pdn */
static uint8_t spm_power_collapse_without_rpm_krait_v3[] __initdata = {
    0x00, 0x24, 0x84, 0x10, 0x09, 0x03, 0x01,
    0x10, 0x84, 0x30, 0x0C, 0x24, 0x30, 0x0f,
};

static uint8_t spm_power_collapse_with_rpm_krait_v3[] __initdata = {
    0x00, 0x24, 0x84, 0x10, 0x09, 0x07, 0x01, 0x0B,
    0x10, 0x84, 0x30, 0x0C, 0x24, 0x30, 0x0f,
};
...
static uint8_t l2_spm_wfi_cmd_sequence[] __initdata = {
    0x00, 0x20, 0x03, 0x20, 0x00, 0x0f,
};

static uint8_t l2_spm_gdhs_cmd_sequence[] __initdata = {
    0x00, 0x20, 0x34, 0x64, 0x48, 0x07, 0x48,
    0x20, 0x50, 0x64, 0x04, 0x34, 0x50, 0x0f,
};
static uint8_t l2_spm_power_off_cmd_sequence[] __initdata = {
    0x00, 0x10, 0x34, 0x64, 0x48, 0x07, 0x48,
    0x10, 0x50, 0x64, 0x04, 0x34, 0x50, 0x0F,
};
...
static void __init apq8064ab_update_krait_spm(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(msm_spm_data); i++) {
    ...
    for (j = 0; j < pdata->num_modes; j++) {
      if (pdata->modes[j].cmd == spm_power_collapse_without_rpm)
        pdata->modes[j].cmd = spm_power_collapse_without_rpm_krait_v3;
      else if (pdata->modes[j].cmd == spm_power_collapse_with_rpm)
        pdata->modes[j].cmd = spm_power_collapse_with_rpm_krait_v3;
    }
  }
}
...
static void __init apq8064_init_buses(void) {
  ...
  msm_bus_8064_apps_fabric.dev.platform_data = &msm_bus_8064_apps_fabric_pdata;
  msm_bus_8064_sys_fabric.dev.platform_data = &msm_bus_8064_sys_fabric_pdata;
  msm_bus_8064_mm_fabric.dev.platform_data = &msm_bus_8064_mm_fabric_pdata;
...
static struct platform_device apq8064_device_ext_5v_vreg __devinitdata = {
    ...
    .dev =
        {
            .platform_data = &apq8064_gpio_regulator_pdata[GPIO_VREG_ID_EXT_5V],
        },
};
...
static struct platform_device apq8064_pm8921_device_rpm_regulator
    __devinitdata = {
        .name = "rpm-regulator",
        .id = 1,
        .dev =
...
static int sec_jack_get_adc_value(void) {
  ...
  rc = pm8xxx_adc_mpp_config_read(PM8XXX_AMUX_MPP_3,
                                  ADC_MPP_1_AMUX6_SCALE_DEFAULT, &result);
  if (rc) {
    pr_err("%s : error reading mpp %d, rc = %d\n", __func__, PM8XXX_AMUX_MPP_3,
           rc);
    return rc;
  }
...
static int uv_get_adc_value(void) {
  ...
  rc = pm8xxx_adc_mpp_config_read(PM8XXX_AMUX_MPP_1, ADC_MPP_2_AMUX6, &result);
  if (rc) {
    pr_err("%s : error reading mpp %d, rc = %d\n", __func__, PM8XXX_AMUX_MPP_1,
           rc);
    return rc;
  }
...
static struct platform_device *pm8921_common_devices[] __initdata = {
    &apq8064_device_ext_5v_vreg,   &apq8064_device_ext_mpp8_vreg,
    &apq8064_device_ext_3p3v_vreg, &apq8064_device_ssbi_pmic1,
...
static struct platform_device mpq8064_device_ext_1p2_buck_vreg __devinitdata = {
    .name = GPIO_REGULATOR_DEV_NAME,
    .id = SX150X_GPIO(4, 2),
    .dev =
        {
...
static struct platform_device mpq8064_device_ext_1p8_buck_vreg __devinitdata = {
    .name = GPIO_REGULATOR_DEV_NAME,
    .id = SX150X_GPIO(4, 4),
    .dev =
        {
...
static struct platform_device mpq8064_device_ext_2p2_buck_vreg __devinitdata = {
    .name = GPIO_REGULATOR_DEV_NAME,
    .id = SX150X_GPIO(4, 14),
    .dev =
        {
...
static struct platform_device mpq8064_device_ext_5v_buck_vreg __devinitdata = {
    .name = GPIO_REGULATOR_DEV_NAME,
    .id = SX150X_GPIO(4, 3),
    .dev =
        {
            .platform_data = &mpq8064_gpio_regulator_pdata[GPIO_VREG_ID_AVC_5V],
        },
};

static struct platform_device mpq8064_device_ext_3p3v_ldo_vreg __devinitdata = {
    .name = GPIO_REGULATOR_DEV_NAME,
    .id = SX150X_GPIO(4, 15),
    .dev =
        {
...
static void es325_2MIC_RST_init(void) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_2MIC_RST),
                     &es325_2MIC_RST);

  gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_2MIC_RST), 1);

  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_2MIC_WAKE),
                     &es325_2MIC_WAKE);
...
static void __init apq8064_i2c_init(void) {
  ...
  mpq8064_device_qup_i2c_gsbi5.dev.platform_data = &mpq8064_i2c_qup_gsbi5_pdata;
  apq8064_device_qup_i2c_gsbi2.dev.platform_data = &apq8064_i2c_qup_gsbi2_pdata;
  apq8064_device_qup_i2c_gsbi7.dev.platform_data = &apq8064_i2c_qup_gsbi7_pdata;
}
...
#else
static int ethernet_init(void) { return 0; }
#endif
...
static const unsigned int mpq_keymap[] = {
    KEY(0, 0, KEY_UP),    KEY(0, 1, KEY_ENTER), KEY(0, 2, KEY_3),

    KEY(1, 0, KEY_DOWN),  KEY(1, 1, KEY_EXIT),  KEY(1, 2, KEY_4),

    KEY(2, 0, KEY_LEFT),  KEY(2, 1, KEY_1),     KEY(2, 2, KEY_5),

    KEY(3, 0, KEY_RIGHT), KEY(3, 1, KEY_2),     KEY(3, 2, KEY_6),
};
...
static void __init apq8064_init_dsps(void) {
  struct msm_dsps_platform_data *pdata = msm_dsps_device_8064.dev.platform_data;
...
static struct i2c_registry apq8064_i2c_devices[] __initdata = {
    {I2C_LIQUID, APQ_8064_GSBI1_QUP_I2C_BUS_ID, smb349_charger_i2c_info,
...
static void __init register_i2c_devices(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(mpq8064_i2c_devices); ++i) {
    if (mpq8064_i2c_devices[i].machs & mach_mask)
      i2c_register_board_info(mpq8064_i2c_devices[i].bus,
...
static void main_mic_bias_init(void) {
  ...
  ret = gpio_request(PM8921_GPIO_PM_TO_SYS(PMIC_MAIN_MICBIAS_EN), "micbias_en");
  if (ret) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_MAIN_MICBIAS_EN),
                     &main_micbiase);
}

static void __init gpio_rev_init(void) {
#ifdef CONFIG_SEC_FPGA
  barcode_i2c_gpio_data.sda_pin = PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_SI);
  barcode_i2c_gpio_data.scl_pin = PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_CLK);
  barcode_emul_info.spi_en = -1;

  if (system_rev > BOARD_REV05)
    barcode_emul_info.cresetb = PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_FPGA_CRESET_B);

  barcode_emul_info.spi_clk = PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_CLK);
  barcode_emul_info.spi_si = PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_SI);
#endif
}
static void sec_jack_init(void) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_EAR_MICBIAS_EN),
                     &ear_micbiase);
  ...
  if (system_rev == BOARD_REV08) {
    pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_FSA8048_EN),
                       &fsa8048_en_old);
  }
...
static void vps_sound_init(void) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_VPS_SOUND_EN),
                     &vps_sound_en);
}
...
static void __init apq8064ab_update_retention_spm(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(msm_spm_data); i++) {
    ...
    struct msm_spm_platform_data *pdata = &msm_spm_data[i];
    for (j = 0; j < pdata->num_modes; j++) {
      if (pdata->modes[j].cmd == spm_retention_cmd_sequence)
        pdata->modes[j].cmd = spm_retention_with_krait_v3_cmd_sequence;
    }
  }
}
...
static void __init apq8064_common_init(void) {
  ...
  if (cpu_is_krait_v3()) {
    struct msm_pm_init_data_type *pdata = msm8064_pm_8x60.dev.platform_data;
  ...
  platform_add_devices(early_common_devices, ARRAY_SIZE(early_common_devices));
  if (socinfo_get_pmic_model() != PMIC_MODEL_PM8917)
  ...
  rpmrs_level = msm_rpmrs_levels[MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT];
  msm_hsic_pdata.swfi_latency = rpmrs_level.latency_us;
  rpmrs_level = msm_rpmrs_levels[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE];
  ...
  if (machine_is_apq8064_mtp() || machine_is_JF()) {
    if (socinfo_get_platform_subtype() == PLATFORM_SUBTYPE_DSDA2) {
      ...
      sglte_mdm_8064_device.dev.platform_data = &sglte2_mdm_platform_data;
      platform_device_register(&sglte_mdm_8064_device);
      sglte2_qsc_8064_device.dev.platform_data = &sglte2_qsc_platform_data;
      ...
      i2s_mdm_8064_device.dev.platform_data = &mdm_platform_data;
  ...
  msm8960_init_battery();
    ...
    defined(CONFIG_TOUCHSCREEN_ATMEL_MXTS)
  printk(KERN_ERR "[TSP] System revision, LPM mode : %d %d\n", system_rev,
         poweroff_charging);
  if (!poweroff_charging) {
  ...
  /*cypress_gpio_init();*/
  ret =
      pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_TKEY_INT), &tkey_int);
  if (ret)
...
static void __init samsung_jf_init(void) {
  printk(KERN_ERR "hi init jf eur");
  printk(KERN_ERR "apq8064_reserve display_params %s, %s, %s\n",
         prim_panel_name, ext_panel_name, ext_resolution);
  ...
  */
  ...
  if (machine_is_mpq8064_hrd() || machine_is_mpq8064_dtv()) {
    ...
    mpq8064_device_uartdm_gsbi6.dev.platform_data = &mpq8064_gsbi6_uartdm_pdata;
#endif
...
MACHINE_START(JF, "SAMSUNG JF").map_io = apq8064_map_io,
                  .reserve = apq8064_reserve, .init_irq = apq8064_init_irq,
                  .handle_irq = gic_handle_irq, .timer = &msm_timer,
                  .init_machine = samsung_jf_init,
                  .init_early = apq8064_allocate_memory_regions,
                  .init_very_early = apq8064_early_reserve,
                  .restart = msm_restart, MACHINE_END
