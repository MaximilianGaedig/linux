 ...
 * Copyright (c) 2008-2010, 2012, 2013 The Linux Foundation. All rights
 * reserved.
 *
...
enum { CLK_DIS, CLK_ENABLING, CLK_EN, CLK_TO_BE_DIS, CLK_DISABLING };
...
#define DMOV_CHANNEL_DEFAULT_CONF                                              \
  { .sd = 1, .block = 0, .priority = 0 }
#define DMOV_CHANNEL_MODEM_CONF                                                \
  { .sd = 3, .block = 0, .priority = 0 }
#define DMOV_CHANNEL_CONF(secd, blk, pri)                                      \
  { .sd = secd, .block = blk, .priority = pri }

static struct msm_dmov_chan_conf adm0_chan_conf[] = {
    DMOV_CHANNEL_DEFAULT_CONF, DMOV_CHANNEL_DEFAULT_CONF,
    DMOV_CHANNEL_DEFAULT_CONF, DMOV_CHANNEL_DEFAULT_CONF,
    DMOV_CHANNEL_DEFAULT_CONF, DMOV_CHANNEL_DEFAULT_CONF,
    DMOV_CHANNEL_DEFAULT_CONF, DMOV_CHANNEL_DEFAULT_CONF,
    DMOV_CHANNEL_DEFAULT_CONF, DMOV_CHANNEL_DEFAULT_CONF,
    DMOV_CHANNEL_MODEM_CONF,   DMOV_CHANNEL_MODEM_CONF,
    DMOV_CHANNEL_MODEM_CONF,   DMOV_CHANNEL_MODEM_CONF,
    DMOV_CHANNEL_MODEM_CONF,   DMOV_CHANNEL_DEFAULT_CONF,
};
...
#define DMOV_CRCI_DEFAULT_CONF                                                 \
  { .sd = 1, .blk_size = 0 }
#define DMOV_CRCI_CONF(secd, blk)                                              \
  { .sd = secd, .blk_size = blk }

static struct msm_dmov_crci_conf adm0_crci_conf[] = {
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_CONF(1, 4),   DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
...
static struct msm_dmov_crci_conf adm1_crci_conf[] = {
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_CONF(1, 1),   DMOV_CRCI_CONF(1, 1),
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_CONF(1, 1),   DMOV_CRCI_CONF(1, 1),
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF,
    DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_DEFAULT_CONF, DMOV_CRCI_CONF(1, 1),
    DMOV_CRCI_DEFAULT_CONF,
};
...
static struct msm_dmov_conf dmov_conf[] = {
    {
        ...
        .clk_ctl = CLK_DIS,
        .work =
            __DELAYED_WORK_INITIALIZER(dmov_conf[0].work, msm_dmov_clock_work),
    },
    {
        ...
        .clk_ctl = CLK_DIS,
        .work =
            __DELAYED_WORK_INITIALIZER(dmov_conf[1].work, msm_dmov_clock_work),
    }};
#else
static struct msm_dmov_conf dmov_conf[] = {{
    .crci_conf = NULL,
    ...
    .lock = __SPIN_LOCK_UNLOCKED(dmov_lock),
    .clk_ctl = CLK_DIS,
    .work = __DELAYED_WORK_INITIALIZER(dmov_conf[0].work, msm_dmov_clock_work),
}};
#endif

#define MSM_DMOV_ID_COUNT (MSM_DMOV_CHANNEL_COUNT * ARRAY_SIZE(dmov_conf))
#define DMOV_REG(name, adm)                                                    \
  ((name) + (dmov_conf[adm].base) +                                            \
...
static void msm_dmov_enqueue_cmd_ext_work(struct work_struct *work) {
  struct msm_dmov_cmd *cmd = container_of(work, struct msm_dmov_cmd, work);
  unsigned id = cmd->id;
  unsigned status;
  unsigned long flags;
...
static void __msm_dmov_enqueue_cmd_ext(unsigned id, struct msm_dmov_cmd *cmd,
  ...
  case CLK_DISABLING:
    if (onstack)
      INIT_WORK_ONSTACK(&cmd->work, msm_dmov_enqueue_cmd_ext_work);
    else
...
static void dmov_exec_cmdptr_complete_func(struct msm_dmov_cmd *_cmd,
                                           unsigned int result,
                                           struct msm_dmov_errdata *err) {
...
int msm_dmov_exec_cmd(unsigned id, unsigned int cmdptr) {
  ...
  if (cmd.result != 0x80000002) {
    ...
    PRINT_ERROR("dmov_exec_cmdptr(%d):  flush: %x %x %x %x\n", id,
                cmd.err.flush[0], cmd.err.flush[1], cmd.err.flush[2],
                cmd.err.flush[3]);
    return -EIO;
  }
  ...
  while (int_status) {
    ...
    do {
      ...
      if (list_empty(&dmov_conf[adm].active_commands[ch])) {
        ...
        cmd = list_entry(dmov_conf[adm].active_commands[ch].next, typeof(*cmd),
                         list);
      }
...
static void config_datamover(int adm) {
  ...
  for (i = 0; i < MSM_DMOV_CHANNEL_COUNT; i++) {
    ...
    if (chan_conf[i].sd <= 1) {
      ...
      writel_relaxed(conf | DMOV_CONF_SHADOW_EN, DMOV_REG(DMOV_CONF(i), adm));
    }
  }
  for (i = 0; i < MSM_DMOV_CRCI_COUNT; i++) {
    struct msm_dmov_crci_conf *crci_conf = dmov_conf[adm].crci_conf;
...
static int msm_dmov_probe(struct platform_device *pdev) {
  ...
  ret = request_irq(dmov_conf[adm].irq, msm_dmov_isr, 0, "msmdatamover", NULL);
  if (ret) {
    PRINT_ERROR("Requesting ADM%d irq %d failed\n", adm, dmov_conf[adm].irq);
    goto out_wq;
  }
  ...
  for (i = 0; i < MSM_DMOV_CHANNEL_COUNT; i++) {
    ...
    writel_relaxed(DMOV_RSLT_CONF_IRQ_EN | DMOV_RSLT_CONF_FORCE_FLUSH_RSLT,
