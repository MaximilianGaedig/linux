...
unsigned char apq8064_hdmi_as_primary_selected(void) { return hdmi_is_primary; }

unsigned char apq8064_mhl_display_enabled(void) { return mhl_display_enabled; }
...
static int msm_fb_detect_panel(const char *name) {
  ...
  if (machine_is_apq8064_liquid()) {
    ...
    if ((SOCINFO_VERSION_MAJOR(version) == 1) &&
      ...
      if (!strncmp(
              name, MIPI_VIDEO_CHIMEI_WXGA_PANEL_NAME,
              strnlen(MIPI_VIDEO_CHIMEI_WXGA_PANEL_NAME, PANEL_NAME_MAX_LEN)))
        return 0;
    } else {
      if (!strncmp(name, LVDS_CHIMEI_PANEL_NAME,
                   strnlen(LVDS_CHIMEI_PANEL_NAME, PANEL_NAME_MAX_LEN)))
        return 0;
    }
  } else if (machine_is_apq8064_mtp() || machine_is_JF()) {
    if (!strncmp(
            name, MIPI_VIDEO_TOSHIBA_WSVGA_PANEL_NAME,
            strnlen(MIPI_VIDEO_TOSHIBA_WSVGA_PANEL_NAME, PANEL_NAME_MAX_LEN)))
      return 0;
    ...
    if (!strncmp(name, LVDS_FRC_PANEL_NAME,
                 strnlen(LVDS_FRC_PANEL_NAME, PANEL_NAME_MAX_LEN))) {
...
static int mipi_dsi_power_tft_request(void) {
  ...
  if (system_rev < 10) {
    ...
    gpio_tlmm_config(GPIO_CFG(LCD_22V_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_2MA),
                     GPIO_CFG_ENABLE);
  }
#else
  ...
  pr_info("[lcd] configure LCD_22V_EN\n");
  gpio_tlmm_config(
      GPIO_CFG(LCD_22V_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
      GPIO_CFG_ENABLE);
...
static int mipi_dsi_power_octa_request(void) {
  ...
  reg_L30 = regulator_get(&msm_mipi_dsi1_device.dev, "3000mV_LCD");
...
#else
    /* VDD3 */
    reg_L15 = regulator_get(&msm_mipi_dsi1_device.dev, "1800mV_LCD");
...
static int lvds_panel_power(int on) {
  ...
  if (!lvds_power_on) {
    reg_lvs7 = regulator_get(&msm_lvds_device.dev, "lvds_vdda");
    if (IS_ERR_OR_NULL(reg_lvs7)) {
      pr_err("could not get 8921_lvs7, rc = %ld\n", PTR_ERR(reg_lvs7));
      return -ENODEV;
    }

    reg_l2 = regulator_get(&msm_lvds_device.dev, "lvds_pll_vdda");
    if (IS_ERR_OR_NULL(reg_l2)) {
    ...
    reg_ext_3p3v = regulator_get(&msm_lvds_device.dev, "lvds_vccs_3p3v");
...
static int lvds_pixel_remap(void) {
  ...
  if (machine_is_apq8064_cdp() || machine_is_apq8064_liquid()) {
    if ((SOCINFO_VERSION_MAJOR(ver) == 1) && (SOCINFO_VERSION_MINOR(ver) == 0))
...
static struct lcdc_platform_data lvds_pdata = {
    .lcdc_power_save = lvds_panel_power, .lvds_pixel_remap = lvds_pixel_remap};
...
static int hdmi_panel_power(int on) {
  ...
  printk(KERN_ERR "%s: HDMI Core: %s\n", __func__, (on ? "ON" : "OFF"));
  rc = hdmi_core_power(on, 1);
  if (rc)
    rc = hdmi_cec_power(on);

  printk(KERN_ERR "%s: HDMI Core: %s Success\n", __func__, (on ? "ON" : "OFF"));
  return rc;
}
...
static int hdmi_core_power(int on, int show) {
  ...
  /* TBD: PM8921 regulator instead of 8901 */
  if (!reg_ext_3p3v) {
    reg_ext_3p3v = regulator_get(&hdmi_msm_device.dev, "hdmi_mux_vdd");
    if (IS_ERR_OR_NULL(reg_ext_3p3v)) {
      pr_err("could not get reg_ext_3p3v, rc = %ld\n", PTR_ERR(reg_ext_3p3v));
      reg_ext_3p3v = NULL;
      return -ENODEV;
  ...
  if (!reg_8921_lvs7) {
    reg_8921_lvs7 = regulator_get(&hdmi_msm_device.dev, "hdmi_vdda");
  ...
  if (!reg_8921_s4) {
    reg_8921_s4 = regulator_get(&hdmi_msm_device.dev, "hdmi_lvl_tsl");
...
void __init apq8064_set_display_params(char *prim_panel, char *ext_panel,
  ...
  printk(KERN_INFO "!!!!!!!!!!!!!!!!!!!!!!!!!!apq8064_set_display_params!!!!!!!"
                   "!!!!!!!!!!!!!\n");
  /*
  ...
  if (machine_is_mpq8064_hrd() || machine_is_mpq8064_cdp()) {
    printk(KERN_INFO "HDMI is the primary display by default for MPQ\n");
    printk(KERN_ERR "HDMI is the primary display by default for MPQ\n");
  ...
  if (strnlen(prim_panel, PANEL_NAME_MAX_LEN)) {
    strlcpy(msm_fb_pdata.prim_panel_name, prim_panel, PANEL_NAME_MAX_LEN);
    printk(KERN_INFO "msm_fb_pdata.prim_panel_name %s\n",
           msm_fb_pdata.prim_panel_name);
    printk(KERN_ERR "msm_fb_pdata.prim_panel_name %s\n",
           msm_fb_pdata.prim_panel_name);

    if (!strncmp((char *)msm_fb_pdata.prim_panel_name, HDMI_PANEL_NAME,
                 strnlen(HDMI_PANEL_NAME, PANEL_NAME_MAX_LEN))) {
      printk("HDMI is the primary display by"
             " boot parameter\n");
      printk(KERN_ERR "HDMI is the primary display by"
                      " boot parameter\n");
      hdmi_is_primary = 1;
      set_mdp_clocks_for_wuxga();
    }
  ...
  if (strnlen(ext_panel, PANEL_NAME_MAX_LEN)) {
    strlcpy(msm_fb_pdata.ext_panel_name, ext_panel, PANEL_NAME_MAX_LEN);
    printk(KERN_INFO "msm_fb_pdata.ext_panel_name %s\n",
           msm_fb_pdata.ext_panel_name);
    printk(KERN_ERR "msm_fb_pdata.ext_panel_name %s\n",
           msm_fb_pdata.ext_panel_name);

    if (!strncmp((char *)msm_fb_pdata.ext_panel_name, MHL_PANEL_NAME,
                 strnlen(MHL_PANEL_NAME, PANEL_NAME_MAX_LEN))) {
      printk(KERN_INFO "MHL is external display by boot parameter\n");
      printk(KERN_ERR "MHL is external display by boot parameter\n");
