...
static int initialize_ssp_gpio(void) {
  ...
  gpio_tlmm_config(
      GPIO_CFG(GPIO_MCU_CHG, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
      1);
...
static int ssp_read_chg() { return gpio_get_value(GPIO_MCU_CHG); }
...
static void irda_led_poweron(int onoff) { gpio_set_value(GPIO_IRDA_EN, onoff); }
...
static void barcode_gpio_config(void) {
  ...
  pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(PMIC_GPIO_FPGA_CRESET_B), &creset_b);
