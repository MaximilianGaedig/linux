...
static int tracectr_notifier(struct notifier_block *self, unsigned long cmd,
  ...
  if (per_cpu(old_pid, cpu) != -1) {
    if (per_cpu(hotplug_flag, cpu) == 1) {
      ...
      trace_sched_switch_with_ctrs(per_cpu(old_pid, cpu), current_pid);
  }
...
int __init init_tracecounters(void) {
  ...
  if (!dir)
    return -ENOMEM;
  file = debugfs_create_file("enabled", 0660, dir, &value, &fops_perftp);
  if (!file) {
    debugfs_remove(dir);
    return -ENOMEM;
  }
  register_cpu_notifier(&tracectr_cpu_hotplug_notifier_block);
  for_each_possible_cpu(cpu) per_cpu(old_pid, cpu) = -1;
  return 0;
}
