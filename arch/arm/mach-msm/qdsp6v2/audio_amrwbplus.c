...
static void config_debug_fs(struct q6audio_aio *audio) {
  if (audio != NULL) {
    ...
    snprintf(name, sizeof(name), "msm_amrwbplus_%04x", audio->ac->session);
    audio->dentry =
        debugfs_create_file(name, S_IFREG | S_IRUGO, NULL, (void *)audio,
...
static long audio_ioctl(struct file *file, unsigned int cmd,
  ...
  case AUDIO_START: {
    ...
    if (audio->feedback == NON_TUNNEL_MODE) {
      ...
      rc = q6asm_enc_cfg_blk_pcm(audio->ac, audio->pcm_cfg.sample_rate,
    ...
    q6_amrwbplus_cfg.size_bytes = amrwbplus_drv_config->size_bytes;
    q6_amrwbplus_cfg.version = amrwbplus_drv_config->version;
    q6_amrwbplus_cfg.num_channels = amrwbplus_drv_config->num_channels;
    q6_amrwbplus_cfg.amr_band_mode = amrwbplus_drv_config->amr_band_mode;
    q6_amrwbplus_cfg.amr_dtx_mode = amrwbplus_drv_config->amr_dtx_mode;
    q6_amrwbplus_cfg.amr_frame_fmt = amrwbplus_drv_config->amr_frame_fmt;
    q6_amrwbplus_cfg.amr_lsf_idx = amrwbplus_drv_config->amr_lsf_idx;

    rc = q6asm_media_format_block_amrwbplus(audio->ac, &q6_amrwbplus_cfg);
    if (rc < 0) {
...
static int audio_open(struct inode *inode, struct file *file) {
  ...
  if (audio->codec_cfg == NULL) {
    pr_err("%s:failed kzalloc for amrwb+ config structure", __func__);
    kfree(audio);
    return -ENOMEM;
  }
  audio->pcm_cfg.buffer_size = PCM_BUFSZ_MIN;

  audio->ac = q6asm_audio_client_alloc((app_cb)q6_audio_cb, (void *)audio);
  ...
  if ((file->f_mode & FMODE_WRITE) && (file->f_mode & FMODE_READ)) {
    rc =
        q6asm_open_read_write(audio->ac, FORMAT_LINEAR_PCM, FORMAT_AMR_WB_PLUS);
    if (rc < 0) {
  ...
  pr_debug("%s: AMRWBPLUS dec success mode[%d]session[%d]\n", __func__,
           audio->feedback, audio->ac->session);
  return 0;
fail:
